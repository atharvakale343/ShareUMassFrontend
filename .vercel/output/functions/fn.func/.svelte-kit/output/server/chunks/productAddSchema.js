...
import { j as set_current_component, r as run_all, f as current_component, o as onDestroy, b as get_store_value } from "./lifecycle.js";
...
function superForm(form, formOptions) {
  ...
  {
    if (options.legacy ?? legacyMode$1) {
  ...
  async function Form_validate(opts = {}) {
    ...
    let errors = {};
    let status;
    const validator2 = opts.adapter ?? options.validators;
...
const formSchema = z.object({
  ...
  image: z.instanceof(File, { message: "Please upload a file." }).refine((f) => f.size < 1e7, "Max 10 MB upload size.").nullable(),
  categories: z.array(z.enum(Object.keys(categories))).min(1, "Please select at least one category."),
  location: z.enum(Object.keys(residentialAreas)),
  price: z.coerce.number().min(1, "Price must be at least $1.00").max(1e6, "Price must be less than $1,000,000"),
  contact: z.string().email("Invalid email or phone number").or(z.string().min(10).refine(validator.isMobilePhone)),
  description: z.string().min(5).max(1e3)
});
export {
  zod as a,
  mapErrors as b,
